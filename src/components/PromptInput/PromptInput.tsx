...
function PromptInput({
  ...
  const standalonePromptColor = useAppState(s => s.standaloneAgentContext?.color);
  ...
  const reservedCompanionColumns = companionReservedColumns(columns, companionSpeaking);
  const textInputColumns = columns - 3 - reservedCompanionColumns;
  const promptLineWidth = Math.max(0, columns - reservedCompanionColumns);
  ...
  const getPromptBorderStyle = (): {
    ...
    if (teammateColorName && AGENT_COLORS.includes(teammateColorName as AgentColorName)) {
      return {
        color: AGENT_COLOR_TO_THEME_COLOR[teammateColorName as AgentColorName],
        rainbow: false
      };
    }
    if (standalonePromptColor === 'rainbow') {
      return {
        color: 'promptBorder',
        rainbow: true
      };
    }
    if (standalonePromptColor && AGENT_COLORS.includes(standalonePromptColor as AgentColorName)) {
      return {
        color: AGENT_COLOR_TO_THEME_COLOR[standalonePromptColor as AgentColorName],
        rainbow: false
      };
    }
    return {
  ...
  const {
    color: promptBorderColor,
    rainbow: useRainbowBorder
  } = getPromptBorderStyle();
  const promptBorderText = buildBorderText(showFastIcon ?? false, showFastIconHint, fastModeCooldown);
  if (isExternalEditorActive) {
    return <Box flexDirection="row" alignItems="center" justifyContent="center" borderColor={promptBorderColor} borderStyle="round" borderLeft={false} borderRight={false} borderBottom width="100%">
  ...
  return <Box flexDirection="column" marginTop={briefOwnsGap ? 0 : 1}>
      ...
      {swarmBanner ? <>
          ...
          <Box flexDirection="row">{renderRainbowRule(promptLineWidth, 3)}</Box>
        </> : <Box flexDirection="row" alignItems="flex-start" justifyContent="flex-start" borderColor={promptBorderColor} borderStyle="round" borderLeft={false} borderRight={false} borderBottom width="100%" borderText={promptBorderText}>
          <PromptInputModeIndicator mode={mode} isLoading={isLoading} viewingAgentName={viewingAgentName} viewingAgentColor={viewingAgentColor} />
          <Box flexGrow={1} flexShrink={1} onClick={handleInputClick}>
            {textInputElement}
          </Box>
...
function renderRainbowRule(width: number, phase: number = 0, suffix?: string): React.ReactNode {
  const suffixText = suffix ? stripAnsi(suffix) : '';
  const ruleWidth = Math.max(0, width - stringWidth(suffixText));
  return <>
      {Array.from({
      length: ruleWidth
