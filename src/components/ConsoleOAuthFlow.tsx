...
export function ConsoleOAuthFlow({
  ...
  const handleAPICompatibleProfileSelect = useCallback((profileName: string) => {
    ...
    const result = updateSettingsForSource('userSettings', buildLLMSelectionSettingsPatch(currentSettings, {
      profileName,
      model: null
    }));
    if (result.error) {
...
function OAuthStatusMessage({
  ...
  switch (oauthStatus.state) {
    ...
    case 'platform_setup':
      return (
        <Box flexDirection="column" gap={1} marginTop={1}>
          <Text bold>Using Claude via 3rd-party cloud</Text>
          <Box flexDirection="column" gap={1}>
            <Text>
      ...
      return (
        <Box flexDirection="column" gap={1} marginTop={1}>
          ...
          <Box flexDirection="column" marginTop={1}>
            ...
            <Text dimColor>
              · <Text bold>CLAUDE_CODE_LLM_PROFILE=deepseek bun run dev --bare</Text>
            </Text>
            <Text dimColor>
              · <Text bold>CLAUDE_CODE_LLM_PROFILE=qwen bun run dev --bare</Text>
            </Text>
          </Box>
          <Box marginTop={1}>
            <Select
    ...
    case 'api_compat_missing_env':
      return (
        <Box flexDirection="column" gap={1} marginTop={1}>
          ...
          <Text>
            This provider profile needs <Text bold>{missingApiKeyEnv}</Text> in
            the environment before Claude Code can use it.
          </Text>
          <Text dimColor>
            Bun loads the project .env automatically. Set the key, restart
            Claude Code, then select this profile again.
          </Text>
          <Box flexDirection="column" marginTop={1}>
            <Text dimColor>
            ...
            <Text dimColor>
              ·{' '}
              <Text bold>
                CLAUDE_CODE_LLM_PROFILE={missingProfileName} bun run dev --bare
              </Text>
            </Text>
          </Box>
          <Box marginTop={1}>
            <Text dimColor>
              Press <Text bold>Enter</Text> to go back to provider options.
            </Text>
          </Box>
        </Box>
      );
